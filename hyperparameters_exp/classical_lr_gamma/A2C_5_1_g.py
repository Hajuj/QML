...
class A2C(object):
    def __init__(self, observation_space, action_space):
        ...
        self.lr = 1e-4  # learning rate 0.0005
    ...
    def train(self, env, max_episodes):
        ...
        while episodes < max_episodes:
            ...
            # calculate score average every 100 episodes
            if (episodes % 100) == 0:
                average_score = int(total_score / 100)
                stats.append([episodes, average_score, env_name, max_episodes, self.lr, self.gamma,
                              self.hidden_layers, seed])
                total_score = 0
                # print("Episode:{} \t Average Reward:{}".format(episodes, average_score))

                # save stats to CSV file
                with open(os.path.join(directory, file_name), 'w') as f:
                    writer = csv.writer(f)
                    writer.writerow(
                        ['Episode', 'Average Reward', 'Environment', 'Max Episodes', 'Learning Rate', 'Gamma',
                         'Hidden Layers', 'Seed'])
...
if __name__ == '__main__':
    ...
    for seed in range(10):
        ...
        file_name = "A2C_5_1_g.csv".split(".")[0] + '_' + datetime.datetime.now().strftime("%d.%m.%Y_%H-%M-%S") + ".csv"
