...
def qnode(inputs, weights):
    ...
    for j in range(2):
        ...
        # rotations
        for i in range(4):
            qml.RY(weights[i, j, 0], wires=i)
            qml.RZ(weights[i, j, 1], wires=i)
            qml.RY(weights[i, j, 2], wires=i)

    return [qml.expval(qml.PauliZ(i)) for i in range(4)]  # measure


# The Actor (Policy) as a VQC with Post Processing Neural Network
class QuantumActor(nn.Module):
    def __init__(self, observation_space, action_space):
        ...
        self.l1 = qml.qnn.TorchLayer(qnode, weight_shapes)  # VQC layer
        self.l2 = nn.Linear(4, action_space)  # hidden layers, output layer (actions)

    def forward(self, state):
        expval = self.l1(state)
        prob0 = (expval + 1) / 2  # probability to measure 0
        state = F.relu(prob0)  # activation function RelU
...
# The Critic (Value) as a VQC with Post Processing Neural Network
class QuantumCritic(nn.Module):
    def __init__(self, observation_space):
        ...
        self.l1 = qml.qnn.TorchLayer(qnode, weight_shapes)  # VQC layer
        self.l2 = nn.Linear(4, 1)  # hidden layers, output layer
...
class A2C(object):
    def __init__(self, observation_space, action_space):
        ...
        # init actor
        self.actor = QuantumActor(observation_space, action_space)
        # optimize actor
...
if __name__ == '__main__':
    ...
    for seed in range(10):
        ...
        file_name = "Q2Q_5.csv".split(".")[0] + "_{}".format(seed) + '_' + datetime.datetime.now().strftime("%d.%m.%Y_%H-%M-%S") + ".csv"
